...
export default function Nav({show, onClose}) {
  ...
  return (
    <aside aria-label="Nawigacja panelu administratora" className={(show?'left-0':'-left-full')+" top-0 text-gray-300 p-4 fixed w-[85vw] max-w-xs bg-ink h-full md:sticky md:top-2 md:self-start md:h-[calc(100vh-1rem)] md:w-auto md:rounded-2xl md:m-2 md:overflow-y-auto transition-all z-50 shadow-xl md:shadow-none"}>
      ...
      <nav className="flex flex-col gap-2">
        ...
        {isAdmin && (
          <Link href={'/team'} onClick={onClose} className={pathname.includes('/team') ? activeLink : inactiveLink}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={pathname.includes('/team') ? activeIcon : inactiveIcon}>
